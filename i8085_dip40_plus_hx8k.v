// Intel 8085 Enhanced DIP40 Configuration for iCE40 HX8K
// Same as dip40_plus but uses EBR (14KB) instead of SPRAM (128KB)
// HX8K has 32 EBRs; 4 used by SPI cache, leaving 28 for RAM
//
// Memory Map:
//   0x0000-0x37FF: Internal EBR RAM (14KB, 28 blocks)
//   0x3800-0x3FFF: EXTERNAL (2KB window for peripherals)
//   0x4000-0xFFFF: Internal SPI flash cache (48KB visible, banked)
//
// I/O Ports:
//   0xF0: ROM bank register (8-bit, 256 banks x 48KB = 12MB)
//   (No RAM banking - only 14KB total)

module i8085_dip40_plus_hx8k #(
    parameter [15:0] EXT_WINDOW_BASE = 16'h3800,  // Top of 14KB RAM
    ...
    // =========================================================================
    // EBR RAM - 28 blocks x 512 bytes = 14KB
    // Using 256x16 mode for each block (HX8K has 32 EBRs, 4 used by cache)
    // =========================================================================

    reg  [7:0]  block_addr;      // Address within block (256 words = 512 bytes)
    reg  [4:0]  block_sel;       // Which of 28 blocks
    reg  [15:0] ram_wdata;
    reg         ram_we;
    wire [15:0] ebr_rdata [0:27];
    wire [27:0] ebr_we;

    genvar i;
    generate
        for (i = 0; i < 28; i = i + 1) begin : ebr_gen
    ...
    // Mux read data from selected block
    reg [4:0] block_sel_r;
    always @(posedge clk) block_sel_r <= block_sel;

    wire [15:0] ram_rdata = ebr_rdata[block_sel_r];

    // =========================================================================
    // Bank Register (ROM only - no RAM banking with 16KB)
    // =========================================================================

    reg  [7:0]  rom_bank_reg;     // ROM bank (256 x 48KB = 12MB) - port 0xF0
    ...
    // =========================================================================
    // Address Decode - 16KB RAM, then external window, then ROM
    // =========================================================================

    task set_addr_decode;
        input [15:0] addr;
        begin
            // EBR addressing: 32 blocks of 256 words (512 bytes each)
            // addr[13:9] = block select, addr[8:1] = word address, addr[0] = byte select
            block_sel <= addr[13:9];
            block_addr <= addr[8:1];
            ext_addr_reg <= addr;

            if (is_external(addr)) begin
                // External window (0x3C00-0x3FFF by default)
                ram_cs <= 1'b0;
                rom_cs <= 1'b0;
                ext_cs <= 1'b1;
            end else if (addr[15:14] == 2'b00) begin
                // Lower 16KB: Internal EBR RAM
                ram_cs <= 1'b1;
                rom_cs <= 1'b0;
                ext_cs <= 1'b0;
            end else begin
                // Upper 48KB: ROM from SPI flash
                ram_cs <= 1'b0;
                rom_cs <= 1'b1;
                rom_rd_reg <= 1'b1;
                ext_cs <= 1'b0;
            ...
            case (fsm_state)
                ...
                S_EXECUTE: begin
                    ...
                    if (cpu_stack_wr) begin
                        ...
                        // No RAM banking on HX8K version
                        fsm_state <= S_FETCH_OP;
                    end else begin
                        fsm_state <= S_FETCH_OP;
                    end
