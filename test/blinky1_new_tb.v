...
module blinky1_ram_tb;
    ...
    // We use hierarchical access: dut.mem.ram_bank0.mem[word_addr]
    ...
    initial begin
        ...
        $display("  SPRAM word[0] = 0x%04x (should be 0x1221)", dut.mem.ram_bank0.mem[0]);
        $display("  SPRAM word[1] = 0x%04x (should be 0x363F)", dut.mem.ram_bank0.mem[1]);
        ...
        while (cycle_count < TIMEOUT_CYCLES && toggle_count < 4) begin
            ...
            if ((cycle_count <= 200 && cycle_count % 5 == 0) || (cycle_count % 1000 == 0)) begin
                $display("  cycle %0d: PC=0x%04x fsm=%0d op=0x%02x A=0x%02x B=0x%02x gpio_out=0x%02x gpio_oe=0x%02x",
                         cycle_count, dut.cpu.r_pc, dut.cpu.fsm_state, dut.cpu.fetched_op,
                         dut.cpu.r_a, dut.cpu.r_b, gpio0_out, gpio0_oe);
            end
