...
module i8085sv (
    ...
    // SPRAM Banks - with vmath bus arbitration
    reg  [13:0] ram_addr;
    reg  [15:0] ram_wdata;
    reg  [3:0]  ram_we;
    wire [15:0] ram_rdata;
    ...
    // Forward declarations for vmath bus signals (defined later)
    ...
    // Mux SPRAM signals: vmath takes bus when bus_req is high
    wire [13:0] spram_addr  = vmath_bus_req ? vmath_mem_addr  : ram_addr;
    wire [15:0] spram_wdata = vmath_bus_req ? vmath_mem_wdata : ram_wdata;
    wire [3:0]  spram_we    = vmath_bus_req ? vmath_mem_we    : ram_we;
    wire [1:0]  spram_bank  = vmath_bus_req ? vmath_mem_bank  : ram_bank_reg;
    wire [3:0]  ram_cs = (4'b0001 << spram_bank);

    wire [15:0] ram_rdata_0, ram_rdata_1, ram_rdata_2, ram_rdata_3;

    SB_SPRAM256KA ram_bank0 (
        .ADDRESS(spram_addr), .DATAIN(spram_wdata), .MASKWREN(spram_we),
        .WREN(|spram_we & ram_cs[0]), .CHIPSELECT(ram_cs[0]), .CLOCK(clk),
        .STANDBY(1'b0), .SLEEP(1'b0), .POWEROFF(1'b1), .DATAOUT(ram_rdata_0)
    );
    SB_SPRAM256KA ram_bank1 (
        .ADDRESS(spram_addr), .DATAIN(spram_wdata), .MASKWREN(spram_we),
        .WREN(|spram_we & ram_cs[1]), .CHIPSELECT(ram_cs[1]), .CLOCK(clk),
        .STANDBY(1'b0), .SLEEP(1'b0), .POWEROFF(1'b1), .DATAOUT(ram_rdata_1)
    );
    SB_SPRAM256KA ram_bank2 (
        .ADDRESS(spram_addr), .DATAIN(spram_wdata), .MASKWREN(spram_we),
        .WREN(|spram_we & ram_cs[2]), .CHIPSELECT(ram_cs[2]), .CLOCK(clk),
        .STANDBY(1'b0), .SLEEP(1'b0), .POWEROFF(1'b1), .DATAOUT(ram_rdata_2)
    );
    SB_SPRAM256KA ram_bank3 (
        .ADDRESS(spram_addr), .DATAIN(spram_wdata), .MASKWREN(spram_we),
        .WREN(|spram_we & ram_cs[3]), .CHIPSELECT(ram_cs[3]), .CLOCK(clk),
    ...
    // Vector Math Unit - uses SPRAM for data access
    // Note: vmath bus signals declared earlier for SPRAM muxing
    wire        vmath_busy;
